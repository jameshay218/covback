...
create_many_province_partab <- function(parTab_start, n_provinces,tmax=100){
  ...
  parTab <- NULL
  parTab_province1 <- data.frame(values=c(0.006,0,0,1,0),
                                 names=c("export_prob","growth_rate","import_propn","i0","t0"),
                                 province="1",
                                 fixed=c(0,0,1,1,0),
                                 lower_bound=c(0,0,0,0,0),
                                 upper_bound=c(1,0.2,100,10,1),
                                 steps=0.1,
                                 lower_start=c(0.001,0.10,0,0,0),
                                 upper_start=c(0.01,0.15,100,10,1),stringsAsFactors = FALSE)
  parTab_province1 <- generate_start_tab(parTab_province1)
  for(province in provinces[2:length(provinces)]){
    parTab_province <- data.frame(values=c(0,0,1,0),
                                  names=c("growth_rate","import_propn","i0","t0"),
                                  province=province,
                                  fixed=c(0,0,1,0),
                                  lower_bound=c(0,0,0,0),
                                  upper_bound=c(0.2,1,1,tmax),
                                  steps=0.1,
                                  lower_start=c(0.05,0,5,10),
                                  upper_start=c(0.15,1,1,40),stringsAsFactors = FALSE)
    parTab_province <- generate_start_tab(parTab_province)
    parTab <- bind_rows(parTab, parTab_province)
  }
  total_imports <- parTab %>% filter(names=="import_propn") %>% summarise(x=sum(values)) %>% pull(x)
  parTab[parTab$names == "import_propn","values"] <- parTab[parTab$names == "import_propn","values"]/total_imports
  
  parTab <- bind_rows(parTab_top, parTab_province1, parTab)
  parTab[parTab$names %in% c("import_propn","export_prob"),"fixed"] <- 1
  return(parTab)
}
