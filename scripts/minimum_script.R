...
chain_no <- 1
local_growth <- TRUE
filename1 <- paste0("chain_start",t0,"_ver",model_ver,"_local",local_growth,"s_chain",chain_no)
...
export_probs <- create_export_prob_matrix(total_travellers, wuhan_pop_ini,tmin, tmax)
...
output_univariate <- run_MCMC(parTab=startTab, data=confirmed_data1, mcmcPars=mcmcPars1, filename=filename1,
...
## Run second chain
output_multivariate <- run_MCMC(parTab=startTab, data=confirmed_data1, mcmcPars=mcmcPars, 
                                filename=filename1,
