...
nchains <- 4
filenames <- c("seed_time_1","seed_time_2",
...
prior_func <- create_random_effects_rlocal(parTab, "3",0.4,0.25,"gamma")
...
res <- foreach(i=seq_along(filenames),.packages=c("covback","lazymcmc","tidyverse")) %dopar% {
  setwd("~/Documents/GitHub/covback_chains_final/")
  ...
  if(filename == "seed_time_1"){
    ...
    print("rlocal_prior")
    prior_func1 <- create_random_effects_rlocal(parTab, "3",0.4,0.25,"gamma")
  } else {
    parTab1 <- parTab
  }
